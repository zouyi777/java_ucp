...
function intEvent() {
    ...
    $("#addWorks").click(function () {
        x_admin_show('新增作品','works-add.html','600','500');
    });
}
/**
 * 初始化列表事件
...
function initListItemEvent(){
    ...
    //穿透详情
    $(".worksItem").bind('click',function () {
        let param = {
            "userId":$(this).attr("data-id"),
            "isEdit":false
        }
        ucp.common.setUrlPram(param);
        x_admin_show('作品详情','works-add.html','800','500');
    });

    //删除作品
    $(".worksDlete").bind('click',function () {
    ...
    //编辑
    $(".worksEdit").bind('click',function () {
        let param = {
            "userId":$(this).attr("data-id"),
            "isEdit":true
        }
        ucp.common.setUrlPram(param);
        x_admin_show('修改作品','works-add.html','800','500');
        ucp.common.stopPropagationDefault();
    });
